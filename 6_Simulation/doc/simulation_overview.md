...
| PN + FOV + NMPC | 40.00 | 1.6338 | 404.05 | 0.30 | 0.992 | 0.180 |
...
- **NMPC 可见率最高（0.992），丢失时长仅 0.30 s，控制能量和 yaw rate 均最低**，表现出最强的视场保持和控制平滑性。
- 但 NMPC 在 40 s 内未按 1.5 m 半径完成捕获（最小距离 1.6338 m），说明当前权重偏向视场保持，后续可调整终端距离权重以增强贴近能力。
