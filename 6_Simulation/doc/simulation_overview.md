...
| `guidance.py` | 实现 Direct pursuit、3D PN、PN + FOV、PN + FOV + CBF、PN + FOV + MPPI、PN + FOV + NMPC |
...
- 有 FOV 算法：目标丢失后指向 last-seen 外推预测点。
...
- 验证算法处理目标速度的能力
- 适合比较 Direct pursuit 与 PN/NMPC 在捕获时间和平均距离上的差异
...
### 6.2 目标丢失后的 last-seen 预测

- 目标可见时：使用真实目标状态并刷新最后观测记忆
- 目标不可见时：使用最后一次观测的位置和速度做匀速外推
...
| 算法名称 | 角色 | 主要功能 |
| --- | --- | --- |
| Direct pursuit | 基线算法 | 速度方向始终指向目标当前位置 |
| 3D PN | 经典导引对比 | 使用三维比例导引进行拦截 |
| PN + FOV | 视场约束基线 | 目标离开视场后使用 last-seen 预测 |
...
使用相对位置 `r = p_t - p_p` 和相对速度 `v_rel = v_t - v_p`，通过 LOS 角速度生成横向 PN 加速度。该算法假设目标始终可见。

### 7.3 PN + FOV

在 PN 基础上加入有限视场判断。目标可见时使用真实目标状态；丢失时使用 last-seen 匀速预测点并降低导引增益。是后续 CBF、MPPI、NMPC 的共同基线。
...
- 首先计算 PN 名义控制 `a_PN`
- 构造一组候选加速度（原始 PN、缩放 PN、直接拦截项、末端贴近项、速度匹配项、LOS 垂直方向扰动项）
...
**注意**：Direct pursuit 和 3D PN 按理想传感器处理（`visible_ratio=1`），其视场指标不能直接与 FOV 算法在同一传感器条件下比较。
...
| 3D PN | 6.45 | 0.0016 | 1160.98 | 0.00 | 1.000 | 1.286 |
| PN + FOV | 6.45 | 0.0005 | 747.15 | 30.20 | 0.246 | 1.274 |
| PN + FOV + CBF | 6.45 | 0.0007 | 996.37 | 25.90 | 0.353 | 1.266 |
| PN + FOV + MPPI | 5.65 | 0.0162 | 445.51 | 2.55 | 0.936 | 1.230 |
| **PN + FOV + NMPC** | 7.55 | 0.8895 | **62.49** | **0.00** | **1.000** | **0.015** |

NMPC 在静止目标场景中不再追求最快捕获，而是显著降低控制强度和机头转动：控制能量最低（62.49），目标全程保持可见（可见率 1.000，丢失时长 0.00 s），yaw 角速度最平滑（0.015 rad/s）。其捕获时间为 7.55 s，说明当前参数更偏向视场保持与平滑控制。
...
| 3D PN | 5.75 | 0.0008 | 1187.40 | 0.00 | 1.000 | 1.334 |
| PN + FOV | 5.75 | 0.0003 | 921.34 | 24.65 | 0.385 | 1.322 |
| PN + FOV + CBF | 5.75 | 0.0014 | 1091.41 | 25.15 | 0.372 | 1.293 |
| PN + FOV + MPPI | 5.25 | 0.0371 | 426.79 | 1.15 | 0.971 | 1.334 |
| **PN + FOV + NMPC** | 6.15 | 0.4955 | **53.58** | **0.00** | **1.000** | **0.070** |

NMPC 在直线运动目标下实现了全程可见（可见率 1.000，丢失时长 0.00 s），控制能量最低（53.58），yaw 角速度最平滑（0.070 rad/s）。其捕获时间为 6.15 s，慢于 MPPI 和 PN，体现出以视场保持和低控制能量换取更保守接近过程的特点。
...
| 3D PN | 5.65 | 0.0006 | 1130.97 | 0.00 | 1.000 | 1.213 |
| PN + FOV | 5.65 | 0.0227 | 756.92 | 25.05 | 0.375 | 1.030 |
| PN + FOV + CBF | 5.65 | 0.0064 | 801.40 | 18.65 | 0.534 | 0.908 |
| PN + FOV + MPPI | 5.15 | 0.0295 | 438.47 | 2.85 | 0.929 | 1.107 |
| **PN + FOV + NMPC** | 6.30 | 0.6519 | **124.97** | **0.00** | **1.000** | **0.226** |

圆周场景是最具挑战性的测试。NMPC 在该场景中实现全程可见（可见率 1.000，丢失时长 0.00 s），控制能量 124.97 远低于 MPPI 的 438.47，yaw 角速度均值也降至 0.226 rad/s。相应代价是捕获时间为 6.30 s，慢于 MPPI 的 5.15 s，且最小距离不如 PN/MPPI 激进拦截方案。
...
- **Direct pursuit**：简单稳定，但面对机动目标捕获效率低。
- **3D PN**：提高了拦截效率，但未考虑有限视场约束。
- **PN + FOV**：暴露了单纯 PN 在有限视场下长时间失锁的问题。
- **CBF**：能一定程度降低 FOV 丢失，但对复杂机动改善有限。
- **MPPI**：能显著提高可见率并保持较快捕获，但控制能量和 yaw 平滑性不如 NMPC。
- **PN + FOV + NMPC**：在三种场景中均实现 FOV 算法中的最高视场保持能力（当前输出中可见率均为 1.000、丢失时长均为 0.00 s），同时控制能量和 yaw rate 最低；代价是捕获过程更保守，捕获时间和最小距离不一定优于 PN/MPPI。
...
| 算法 | 捕获时间/s | 最小距离/m | 控制能量 | 丢失时长/s | 可见率 | yaw rate mean/rad/s |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Direct pursuit | 11.60 | 0.1495 | 1260.13 | 0.00 | 1.000 | 1.291 |
| 3D PN | 9.80 | 0.0352 | 1235.37 | 0.00 | 1.000 | 1.179 |
| PN + FOV | 9.80 | 0.0092 | 1050.44 | 15.40 | 0.615 | 1.235 |
| PN + FOV + CBF | 15.75 | 0.0325 | 1024.29 | 15.90 | 0.603 | 1.333 |
| PN + FOV + MPPI | 10.00 | 0.0339 | 572.54 | 5.55 | 0.859 | 0.493 |
| PN + FOV + NMPC | 32.60 | 0.7916 | 652.54 | 3.30 | 0.915 | 0.436 |
...
- Direct pursuit 和 PN 在 Gazebo 中捕获时间慢于离线仿真，说明真实飞控闭环引入了动态滞后。
- PN + FOV 视场丢失时长 15.40 s，可见率 0.615，依赖 last-seen 预测仍会产生明显失锁。
- CBF 在该 Gazebo 场景中未显著改善视场保持，丢失时长 15.90 s，可见率 0.603。
- MPPI 丢失时长降至 5.55 s，可见率提高到 0.859。
- **NMPC 在受 FOV 约束算法中可见率最高（0.915），丢失时长最低（3.30 s），yaw rate mean 最低（0.436 rad/s）**，表现出较强的视场保持和控制平滑性。
- 增强末端收敛后，NMPC 在 Gazebo 中可进入 1.5 m 捕获半径，捕获时间为 32.60 s，最小距离约 0.8 m；相较 MPPI，其视场保持和 yaw 平滑性更好，但控制能量更高、捕获时间更长。
