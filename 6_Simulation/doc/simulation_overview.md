...
| **PN + FOV + NMPC** | **5.45** | **0.0001** | **103.51** | 3.00 | 0.925 | **0.169** |

NMPC 在静止目标场景中捕获时间最短（5.45 s），控制能量最低（103.51），yaw 角速度最平滑（0.169 rad/s）。
...
| **PN + FOV + NMPC** | **4.95** | 0.3116 | **86.50** | **0.80** | **0.980** | **0.098** |

NMPC 在直线运动目标下捕获时间最短（4.95 s），可见率 0.980，丢失时长仅 0.80 s，控制能量仅 86.50。
...
| **PN + FOV + NMPC** | **5.15** | 0.4649 | **150.77** | **0.85** | **0.979** | **0.292** |

圆周场景是最具挑战性的测试。NMPC 丢失时长仅 0.85 s，可见率 0.979，控制能量 150.77 远低于 MPPI 的 438.47。
...
- **MPPI**：能显著提高可见率，但控制能量和 yaw 平滑性不如 NMPC。
- **PN + FOV + NMPC**：在三种场景中均表现出低控制能量、低 yaw rate 和高视场保持能力。
