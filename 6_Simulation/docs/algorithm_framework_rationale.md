...
本章针对有限视场条件下的无人机目标追踪仿真问题，说明本项目采用 `PN + FOV + NMPC` 导引框架的原因及其实现依据。该框架的基本功能分工为：PN（Proportional Navigation，比例导引）提供基于相对运动几何的三维拦截指令；FOV（Field of View，视场）约束将视觉传感器可见性纳入导引过程，并通过深度相机位置量测和 α-β 滤波器生成可用目标参考；NMPC（Nonlinear Model Predictive Control，非线性模型预测控制）在有限预测时域内综合考虑目标接近、视场保持和控制平滑性。通过上述组合，仿真系统能够在速度、加速度、高度以及机头转向速率受限的条件下，使追踪机稳定接近运动目标，并降低目标离开视场的概率。
...
仅使用 PN 时，算法隐含假设目标状态始终可用。然而，在视觉追踪任务中，目标可能因相机视场有限、机头转向速率受限或近距离交会等因素离开画面。目标不可见时，真实系统无法继续直接利用当前真实目标状态，只能基于传感器估计状态及短时预测进行导引。因此，本项目将 FOV 可见性和传感器估计共同作为导引计算的一部分。
...
该判据在 `sensor_target_reference()` 中统一计算。对于 FOV 类算法，只有目标处于当前视场内时才允许生成深度相机量测。`depth_camera.py` 中的 `simulate_depth_camera_point()` 会将目标中心表示为机体/相机系三维点，随后 `camera_point_to_world()` 根据追踪机位置和 yaw/pitch 姿态将该量测恢复到地面系。得到的地面系位置量测不会直接替代真实目标状态，而是进入 α-β 滤波器：

```text
p_pred = p_hat + v_hat dt
r = z_p - p_pred
p_hat = p_pred + alpha r
v_hat = v_hat + beta r / dt
```

其中，`z_p` 为深度相机转换到地面系后的目标位置量测，`alpha` 和 `beta` 分别对应 `GuidanceConfig.sensor_ab_alpha` 与 `GuidanceConfig.sensor_ab_beta`。当目标不可见时，算法不偷看真实目标，而是仅对滤波状态做常速度预测：
...
若仿真初始阶段目标不可见且滤波器尚未初始化，则 FOV 类算法返回零加速度，并将 `look_at_position` 设为机头前方搜索点。若滤波器已经初始化但当前不可见，则导引律使用预测目标 `target_reference`，同时对导引加速度乘以 `GuidanceConfig.lost_guidance_gain`，其默认值为 `0.55`：

```text
a = lost_guidance_gain * a_guidance(p_hat_t, v_hat_t)
```

其中 `a_guidance` 可以是 Direct pursuit、PN、CBF、MPPI 或 NMPC 产生的当前导引指令。该处理反映了目标不可见时观测信息不确定性增加的情况，使控制指令相对于可见状态更为保守。

FOV 层还影响机头或相机朝向。`GuidanceResult.look_at_position` 在 FOV 类算法中为当前可用目标参考位置：目标可见时来自深度相机量测更新后的滤波估计，目标不可见时来自滤波器预测。随后 `step_pursuer()` 根据该位置更新 yaw/pitch。因此，FOV 在本项目中不仅是性能评价指标，也直接影响导引输入和朝向参考。
...
在预测窗口内，候选加速度保持不变。对第 `i` 个预测步，目标位置采用当前参考目标状态的常速度模型：

```text
p_t(i) = p_t(0) + v_t i dt_mpc
```

其中，`p_t(0), v_t` 为当前可用的目标参考状态。对于 FOV 类算法，该参考状态来自深度相机量测经 α-β 滤波得到的估计；若目标不可见，则来自滤波器常速度预测。因此，NMPC 的预测过程同样遵守有限视场下的信息可用性约束。
...
随后，主仿真只执行当前仿真步。下一时刻，系统重新获取追踪机状态以及由深度相机量测/α-β 滤波生成的目标参考状态，并重新计算 PN 趋势、生成候选控制、执行滚动预测和代价评估。该 receding horizon 机制使控制器能够根据最新的相对位置、速度、可见性和机头朝向持续修正控制指令。

从 `compute_guidance()` 的调用链可得完整流程：

```text
1. sensor_target_reference() 计算目标当前是否可见，并生成当前可用目标参考。
2. 可见时由深度相机量测和 α-β 滤波更新目标估计；不可见时由滤波器常速度预测。
...
`PN + FOV + NMPC` 适用于本项目的仿真任务，首先是因为该组合与有限视场目标追踪的物理约束一致。目标追踪不仅取决于空间距离，还受传感器朝向和可见性约束影响。PN 基于相对运动几何生成三维拦截指令；FOV 在目标不可见时将导引参考退化为 α-β 滤波器预测结果，避免继续使用理想目标状态；NMPC 则将未来短时间内的视场违反风险纳入当前控制选择。

其次，该组合具有较好的可解释性。PN 公式由相对位置、相对速度和 LOS 角速度确定；FOV 判据由相机前向向量与 LOS 夹角确定；深度相机量测与 α-β 滤波对应有限视场下的目标状态估计；NMPC 代价函数中的各项分别对应距离、视场、控制能量、控制平滑性和 PN 趋势一致性。源码中的 `pn_guidance()`、`fov_visibility()`、`sensor_target_reference()`、`_candidate_accelerations()` 和 `_rollout_cost()` 分别对应上述计算过程，便于对算法行为进行复现和分析。
...
第二，FOV 提供视觉追踪约束。该模块由 yaw/pitch 生成机头或相机前向向量，并通过 LOS 夹角判断目标可见性；当目标可见时，使用深度相机位置量测和 α-β 滤波更新目标估计；当目标丢失时，仅使用滤波器常速度预测，并降低导引增益，以描述有限视场传感器下的信息退化。
